...
4. **Quick Routine** — `src/features/quick/quick-routine.ts`: `QUICK_SESSION_PRESET_ID = 'temp_quick_session'`. Form trên Home (2 stage Work/Break + repeat count, mặc định 25/5 × 4) — không mở Editor. `startPreset({ id: temp_quick_session, name: t('quick.defaultName'), ... })`. Session log presetName = "Quick Routine".
