...
- Remote Config với 8 default values; mọi magic number qua config.
- ATT đúng thời điểm + non-personalized fallback; Privacy Policy Day 1.
...
- Không làm consent SDK (UMP) nâng cao — cơ bản đúng chuẩn + fallback non-personalized là đủ MVP.
...
3. **Remote Config service**: `RemoteConfigService` bọc @react-native-firebase/remote-config, khai báo 8 default values tập trung, `fetchAndActivate()` ở khởi động (timeout ngắn), offline → default local. Không hard-code ngưỡng ở nơi khác. Khi Firebase chưa cấu hình (web/dev chưa có credentials) → fallback default values.
4. **ATT qua `expo-tracking-transparency`**: theo dõi "value-moment" = user đã start timer đầu tiên thành công (flag local). Sau đó mới `requestTrackingPermissionsAsync()`. Denied/Restricted → set non-personalized ads (SDK ads hỗ trợ `setRequestConfiguration`/tag). Ghi `att_status` vào analytics.
...
- **ATT xin quá sớm bị deny cao** → chỉ xin sau value-moment; deny vẫn serve non-personalized (không mất hẳn eCPM).
...
Greenfield — không migrate. Cần tạo Firebase project + đăng ký app Android/iOS, đặt google-services.json/GoogleService-Info.plist, push 8 Remote Config keys lên console trước release (checklist §10 plan).
