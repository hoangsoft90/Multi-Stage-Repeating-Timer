...
- ATT đúng thời điểm + non-personalized fallback (đã implement: `ConsentService` + `resolveNonPersonalized` + `requestNonPersonalizedAdsOnly`); Privacy Policy Day 1.
...
- Không làm consent SDK nâng cao ngoài AdsConsent — UMP cơ bản + fallback non-personalized là đủ MVP (đã implement; form chuẩn Google, không custom).
...
4. **ATT qua `expo-tracking-transparency`**: gọi `requestTrackingPermissionsAsync()` trong `startPreset` (start timer đầu tiên = value-moment; gọi lại các lần sau vô hại vì iOS chỉ prompt một lần). Sau khi trả lời ghi event `att_prompt_shown` (chưa log trạng thái chi tiết `att_status`). Non-personalized fallback qua `ConsentService.shouldUseNonPersonalized()` → `requestNonPersonalizedAdsOnly` trên từng request (helper thuần `resolveNonPersonalized`).
...
7. **Privacy Policy + consent**: URL đặt trong code (hằng số, dễ đổi), link Settings (đã có Phase 1). UMP: dùng module `AdsConsent` sẵn có của react-native-google-mobile-ads (không dependency riêng) — `gatherConsent()` ở bootstrap, `canRequestAds()` gate trước mỗi show, `showPrivacyOptionsForm()` từ Settings.
...
- **ATT xin quá sớm bị deny cao** → chỉ xin sau value-moment; deny vẫn serve non-personalized (không mất hẳn eCPM — đã implement qua `requestNonPersonalizedAdsOnly`).
